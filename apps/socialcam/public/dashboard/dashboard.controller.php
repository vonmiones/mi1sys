...
class RegistryModel
{
	...
	function GetValidatedData()
	{
		return R::getAll( 'SELECT payroll,
							...
							CONCAT(TIMESTAMPDIFF(YEAR,birthdate,CURDATE())," y/o") AS age,barangay,citymun,province,txnstatus,teller 
							FROM entity 
							WHERE txnstatus = "VALIDATED" 
							ORDER BY dtvalidated DESC' );
	}
	...
	function getEntityByUnknownSingle($patname)
	{
	    ...
	    $result = R::getAll("
			...
			OR 	SOUNDEX(REPLACE(last_name,'-','')) = SOUNDEX('$patname')
			OR 	SOUNDEX(REPLACE(fullname_last,'-','')) = SOUNDEX('$patname')
			OR 	REPLACE(last_name,'-','') LIKE '%$patname%'
			OR 	REPLACE(fullname_last,'-','') LIKE '%$patname%'
			OR 	last_name SOUNDS LIKE '$patname'
			OR 	fullname_last SOUNDS LIKE '$patname'
